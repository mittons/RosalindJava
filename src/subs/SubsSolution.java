...
public class SubsSolution {
    ...
    public static void main(String[] args) {
        ...
        List<Integer> patternMatchLocs = kmpStringPatternMatchSearch(inputTextChars, inputPatternChars);
    ...
    //The Knuth-Morris-Pratt algorithm to find the locations where a pattern (e.g. some string) appears as a substring within another string.
    public static List<Integer> kmpStringPatternMatchSearch(char[] stringChars, char[] patternStringChars){
        List<Integer> patternMatchLocs = new ArrayList<>();

        int textLen = stringChars.length;
        int patLen = patternStringChars.length;
        int textIdx = 0;
        int patIdx = 0;
        int[] lspArray = getLspArray(patternStringChars);

        while (textIdx < textLen){
            if (stringChars[textIdx] == patternStringChars[patIdx]){
                textIdx++;
                patIdx++;
                if (patIdx == patLen){
                    //found pattern
